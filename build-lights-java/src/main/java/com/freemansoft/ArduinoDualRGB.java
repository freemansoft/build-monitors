...
/**
 * Arduino controlled pair of Sparkfun RGB LED breakout boards. Arduino running custom firmware with
 * blink support
 * 
 * @author Joe Freeman
 */
public class ArduinoDualRGB implements BuildLightDevice {
    ...
    private void setColor(final int deviceNumber, final int red, final int green, final int blue) {
    ...
    @Override
    public synchronized void bldBlink(final int deviceNumber, final int red, final int green,
                    final int blue) {
    ...
    @Override
    public synchronized void bldSolid(final int deviceNumber, final int red, final int green,
                    final int blue) {
        setColor(deviceNumber, red, green, blue);
        // no blink
    ...
    /**
     * sends a buffer to the device and waits for a response. This firmware always responds with
     * "+<command>" for any command "<command>" that it understands and "-<command>" if it doesn't
     * 
     * @param buffer
     */
    private void sendAndWaitForAck(final byte[] buffer) {
        ...
        while (device.charactersAvailable() < buffer.length + 1) {
            try {
                // we should keep some timer/counter here so we know if we've really waited too long
                // we should probably also see if we got a '-' string back as a nack
    ...
    protected byte convertIntToAsciiChar(final int number) {
        if (number < 0 || number > 15) {
            throw new IllegalArgumentException("number out of single digit hex range " + number);
        }
        byte result;
        if (number > 9) {
