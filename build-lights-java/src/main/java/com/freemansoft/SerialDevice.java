...
/**
 * Serial port wrapper for rxtx. This monitors the inbound connection buffering
 * data that can be picked up later. Writes are blocking meaning the caller will
 * wait until the data is written out
 * 
 * @author Joe Freeman
 */
public class SerialDevice implements SerialPortEventListener {
	 ...
	 * @param parity
	 */
	public SerialDevice(final String portName, final int baudRate,
			final int dataBits, final int stopBits, final int parity) {
		serialPort = findSerialPort(portName, baudRate, dataBits, stopBits,
				parity);
		if (serialPort == null) {
			throw new IllegalStateException("unable to create serial port for "
					+ portName);
		}
		try {
		...
		try {
			...
			LOG.error("too many listeners ", e);
		}
	...
	private SerialPort findSerialPort(final String portName,
			final int baudRate, final int dataBits, final int stopBits,
			final int parity) {
		...
		while (serialPort == null && portList.hasMoreElements()) {
			...
			if (portId.getPortType() == CommPortIdentifier.PORT_SERIAL) {
				...
				if (portId.getName().equals(portName)) {
					...
					try {
						serialPort = (SerialPort) portId.open("SimpleUtil",
								2000);
						try {
							...
							LOG.error("Unable to setup port params ", e);
							serialPort = null;
						}
	...
	public synchronized String getAvailableCharacters() {
		...
		LOG.debug("returning " + extractLength + " characters: '" + extract
				+ "'");
		return extract;
	}
	...
	private void readDataFromPort() {
		...
		try {
			while (inputStream.available() > 0) {
				...
				final String capturedString = new String(readBuffer, 0,
						numBytes);
				capturedCharacters.append(capturedString);
				LOG.debug("numBytes:" + numBytes + " Received:"
						+ capturedCharacters);
			}
		} catch (final IOException e) {
	...
	public void serialEvent(final SerialPortEvent event) {
		...
		case SerialPortEvent.OUTPUT_BUFFER_EMPTY:
			...
			LOG.info("Unexpected receipt of OUTPUT_BUFFER_EMPTY");
			break;
	...
	public void write(final String messageString) {
		LOG.debug("Writing \"" + messageString + "\" to "
				+ serialPort.getName());
	 ...
	 * <p>
	 * This method was added because rxtx changed the behavior between 2.1.7 and
	 * 2.2pre2. It's probably ok because not all devices need to be reset so now
	 * the devices can call this method in their constructor if they need it
	 */
